...
// Recommendation text depends on mode as much as on level: "have a backup
// plan for today" is actionable on location, but meaningless — even
// confusing — two weeks out, when there's nothing to actually change yet.
...
// event name -> level, per the rules table in the product plan.
// Matched case-insensitively against NWS `event`.
...
export interface RuleResult {
  ...
  scopeLabel: string;
  reasons: string[];
  storms: Storm[]; // with distanceKmToTrip filled in
}
...
export function computeThreatLevel(ctx: RuleContext): RuleResult {
  ...
  const relevantAlerts =
    ctx.mode === "on-location" && relevantZones.size > 0
      ...
      : ctx.mode === "before-trip"
        ? ctx.alerts // statewide view before departure
        : [];
  ...
  // Once you're back in LA there's nothing left to monitor — don't let a
  // distant storm or a routine outlook percentage raise the level for a trip
  // that's already over.
  const monitoringActive = ctx.mode !== "outside-hawaii";

  const originPoint =
    ctx.activeSegment?.centroid ?? { lat: 21.3, lon: -157.86 }; // Oahu-ish default before trip
